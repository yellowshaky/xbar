...
```
https://github.com/yellowshaky/xbar.git
```
...
| 용도 | 경로 |
|------|------|
| 소스 / 문서 | `~/Documents/xbar-plugins/` |
| xbar 실행 플러그인 | `~/Library/Application Support/xbar/plugins/` |

### 수정 후 푸시 절차

플러그인 또는 문서를 수정한 경우 아래 절차로 저장소에 반영합니다.

```bash
cd ~/Documents/xbar-plugins
...
> xbar 실행 플러그인(`~/Library/Application Support/xbar/plugins/`)을 직접 수정한 경우,  
> `~/Documents/xbar-plugins/`에도 동일하게 반영 후 커밋할 것.
...
| `sk-hynix-combined.1s.sh` | SK하이닉스 + KODEX + TIGER SK하이닉스레버리지 통합 | 1초 | 항상 |
...
- 엔드포인트: `GET https://wts-info-api.tossinvest.com/api/v3/stock-prices?meta=true&productCodes=A000660,A0193T0,A0195S0`
- 인증: 불필요 (Origin/Referer 헤더만 필요)
- Toss 내부 종목 코드: `A000660` (SK하이닉스), `A0193T0` (KODEX SK하이닉스레버리지), `A0195S0` (TIGER SK하이닉스단일종목레버리지)
